...
## Phase 3 — Write flows (W11) · ◐ PARTIAL 2026-07-30 — Log Activity submit done, see run log
...
| 2026-07-30 | **Backend coverage** | **BE-1/B38's stated impact WITHDRAWN.** Request carries no locale (confirmed live) **but the response comes back correctly French** — the backend resolves language from the **account**. Real defect re-scoped to **two sources of truth**, which — with **B11** (preference not persisted) — is now the **best lead on B25**. **B8 reclassified to backend** (`Minutes Actives` wrong beside `Minutes actives` correct, *same payload*). **B9 confirmed backend-served**. **New BE-24** (French `1 jours` → shared template with no n=1 rule). French captured; 4 of 16. |
| 2026-07-30 | **Phase 3 — write flows** | **W13 CLOSED by one real write** (authorised; debt recorded first; record **confirmed not deletable**). `POST /activity/save` → 200, record created, modal closed, **0 toasts** with observer installed beforehand + 2.5 s wait. **So there is no success toast to localize** — the gap cannot be a translation defect. **B31 widened** to *"the app never confirms or denies a write"* — success and failure are indistinguishable. **New B40 (P2):** `5,000 m` in a French session, where comma is the decimal separator — a French user misreads their own distance by **1000×**; also two number conventions and two distance units on one screen. Caught a near-miss: the first evidence screenshot was taken mid-load and showed only skeletons. |
