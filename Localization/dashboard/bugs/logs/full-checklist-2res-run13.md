...
> ℹ️ **PARTIAL / HISTORICAL.** This run *started* the full-checklist re-run; it was completed by later
> runs (see `ui-break-sweep-de.md`, `all-18-languages.md`, `deep-tier-*.md`, `u9-register-glossary-all-languages.md`).
> Its findings are valid but its coverage table is not current — the current view is
> **[`../00-INDEX.md`](../00-INDEX.md)**.

**Why this run exists.** Runs 5–12 covered roughly **3 of ~22 checklist dimensions** (U1 strings, U4 layout,
U6 glyphs) and the findings were never written into `bug-log.md` or the per-module bug files. This
