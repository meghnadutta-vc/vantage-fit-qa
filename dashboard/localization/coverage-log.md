...
| 3 | 2026-07-10 | French targeted pass (Overview, Create Challenge, Publish Notifications) | fr | Chromium (MCP) | 🟡 | AC1 PASS; French fluent + accurate, **formal vous** (correct) → reinforces #25 that Spanish tú is the outlier. Confirmed #1,2,6,7,8 recur; #12/#15 English by construction (hardcoded bundle literals). No new bugs. Space-before-`:!?` not exercisable on checked screens. Evidence in evidence/fr/ |
| 6 | 2026-07-13 | **Polish — Overview** | pl | Chromium (MCP) | ✅ | pl.json 973/982 translated (0 missing). AC1 PASS, nav fully Polish, no layout breakage, diacritics OK. Same wire-up bugs recur (#1,2,3,8); #5 corrected → backend (config plan.name). Evidence in evidence/pl/ |
| 6 | 2026-07-13 | **Polish — Create Challenge** | pl | Chromium (MCP) | ✅ | Renders Polish when applied. Same FE bugs: #6 (cards English, pl.json has "Niestandardowe wyzwanie"), #7 (concatenation "Stwórz własne Nowe wyzwania"). Builder opened, not field-by-field re-verified. N7: intermittent language reversion pl→en on nav (not reproducible on reload). Evidence evidence/pl/create-challenge_landing.png |
| 4 | 2026-07-10 | **Functional + UI pass** (Overview, Challenges builder, Programs/Content, Reports/Employee) | de | Chromium (MCP) | 🟡 | Behaviors WORK: validation gating, multi-step nav, accented input, search filter (25→4), char counter, column picker, export menu. Bug #26 (two create choosers, one EN). #20 reclassified FE. Full save→delete blocked (image upload / publish-notifies). 5 modules not yet functionally walked. Evidence in evidence/functional/ |
| 4 | 2026-07-10 | **Functional + UI pass — remaining 5** (Community, Communications, Workforce Health, Rewards, Configuration) | de | Chromium (MCP) | ✅ | All 9 modules now walked. Verified: dynamic FAQ add (1→2), char counter (31/60), live preview, period toggle (Monatlich active), radio select (Anerkennungen active), validation gating ×7 forms. Bug #27 (Send enables with just a title → accidental-send-to-all risk). Settings toggles NOT flipped (org-wide 🔴). No crashes. |
