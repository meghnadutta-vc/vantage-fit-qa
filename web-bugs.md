...
stay in their own logs). Compiled 2026-07-24. Split into **Frontend** and **Backend** sections.
...
| Quick Add | #43 | Log Smoking never reflected in web UI (no Vitals row/Trends/pre-fill) — **backend write is OK (200); the missing read path is frontend** | Functional/Data | smoking_02_no_selected.png |
...
> **These are backend bugs** — root cause is in the API/server (data returned, endpoint behaviour, or
> asset serving), so they need a backend developer rather than the frontend team.

| Priority | Area | ID | Title | Evidence |
|---|---|---|---|---|
| P2 (candidate P1) | Create Content | #6 | Employee bite-size viewer is blank — content images **404 with malformed/duplicated CDN paths**, response sends **X-Frame-Options: deny**, and the API uses **byteContentId vs biteContentId** param mismatch | 14_employee_bite_view_attempt.png |
| P4 / Enhancement | Diary | #5 | Weight display disagrees across Diary / Summary / Log-Weight modal because **two backend endpoints** (`app/home` vs `today/overview`) use different "current weight" query semantics | app/home vs today/overview API bodies |
| P1 (disputed — verify) | Diary | #1 | Water intake per-day reset / suspected cross-day misattribution — originally diagnosed backend, but **could not be reproduced on re-test** and was downgraded to a unit-label + goal-mismatch display issue. Re-confirm whether any server-side misattribution remains | diary_01_water_stale_today.png |

*Note: Quick Add #43 (Log Smoking) is listed under Frontend — its backend write succeeds (HTTP 200 with a
real userActivityId); the defect is the missing frontend read/display path.*
...
| BUG-05 | Diary #5 (weight inconsistent across views — backend) |
